...
class Apriori {
    ...
    private function getDatabaseData(){
        $bdd = new PDO('mysql:host=localhost;dbname=movieclick;charset=utf8', 'root', 'root');

        //Il faut recuperer le nombre total de produit

        $nbpaniersrep = $bdd->query('SELECT DISTINCT(CustomerID)  FROM movies');
		$i=0;
		  while (  $nbpan = $nbpaniersrep->fetch()) {
           // $reqSupport = $bdd->query('SELECT count(*) FROM comporte WHERE id_produit='.$i.';');
     #   for($i=1; $i<$nbpan+1;$i++) {
			
            $req_nom_prod = $bdd->query("select *,replace(`Movie`, ',', '') as p FROM movies  WHERE CustomerID  =".$nbpan['CustomerID'].";");
            while ($donnees = $req_nom_prod->fetch()) {
                $tabSupport[$i][] =  $donnees['p'];
            }
    ...
    private function getDatabaseData_actor(){
        $bdd = new PDO('mysql:host=localhost;dbname=movieclick;charset=utf8', 'root', 'root');

        //Il faut recuperer le nombre total de produit

        $nbpaniersrep = $bdd->query('SELECT DISTINCT(CustomerID)  FROM actors ' );
		$i=0;
		  while (  $nbpan = $nbpaniersrep->fetch()) {
           // $reqSupport = $bdd->query('SELECT count(*) FROM comporte WHERE id_produit='.$i.';');
     #   for($i=1; $i<$nbpan+1;$i++) {
			
            $req_nom_prod = $bdd->query("select *,replace(`Actor`, ',', '') as p FROM actors  WHERE CustomerID  =".$nbpan['CustomerID']." ;");
            while ($donnees = $req_nom_prod->fetch()) {
                $tabSupport[$i][] =  $donnees['p'];
            }
    ...
    //Fonction principale
    public function process($db,$choose)
    {
		if($choose=="movie")
        {$tabData=$this->getDatabaseData(); }
	elseif($choose=="actor"){
		$tabData=$this->getDatabaseData_actor();}
        $checked = array();
        $result = array();
